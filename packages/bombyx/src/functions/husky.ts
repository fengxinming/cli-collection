...
async function addCommitHook(
  ...
  return outputFile(filePath, content, 'utf-8');
}
...
export default async function doHusky(
  ...
  // 版本过低
    ...
    if (huskyrc) {
      backupFile(huskyrc, dir).then(() => {
        return unlink(join(dir, huskyrc));
      });
    }
    const { husky } = pkg;
    if (husky) {
      updateFile('husky.bak', husky, dir);
      delete pkg.husky;
    }
  }
  ...
  await runNpmPkg([`husky@${devDependencies.husky.match(/\d/)[0]}`], !existsSync(join(dir, 'node_modules', 'husky')));
